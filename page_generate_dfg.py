...
description.write("")
description.write(
    "**Explore a directly-follows graph (DFG)**"
    ""
    "\n\nActivities are nodes, and arrows "
...
with st.sidebar:
    ...
    show_occupancy = st.checkbox(
        "Show queue and resource occupancy",
        False,
        key="dfg_show_occupancy",
    ...
    with st.expander("ADVANCED: Labels and infrequent paths"):
        occupancy_interval = st.slider(
            "Occupancy snapshot interval (mins)",
            1,
            15,
            1,
            key="dfg_occupancy_interval",
            disabled=not show_occupancy,
            help="Smaller intervals capture more detail; larger intervals calculate faster.",
        )
        dashed = st.checkbox(
            "Dash infrequent paths",
            True,
            key="dfg_dashed",
        ...
        dash_threshold = st.slider(
            "Infrequent path probability threshold",
            0.0,
            1.0,
            0.1,
            step=0.05,
            key="dfg_dash_threshold",
...
with tab_build:
    ...
    graph_parameters = dict(
        ...
        show_transition_probabilities=show_probabilities,
        show_metric=show_metric,
        dashed_infrequent_paths=dashed,
        infrequent_path_dash_threshold=dash_threshold,
        wrap_node_labels=wrap,
        wrap_node_labels_at=wrap_at,
...
with tab_run:
    ...
    tab_ex_2_dfg.info(
        ...
        "compare the results. Make sure 'Show activity counts', 'Show transition "
    ...
    if visible_edges.empty:
        st.info(
            "No transitions meet these filters. Lower the minimum count or probability to show arrows."
        )
    st.write(
        "**The graph refreshes automatically as you change parameters in the sidebar.**"
    )
    st.caption(
        "Scroll inside the viewer to follow the path. Adjust zoom to see more detail or more of the graph."
    )
    zoom = st.slider(
        "Graph zoom (%)",
        50,
    ...
    with st.expander("Explore the discovered nodes and edges that power the DFG"):
        st.subheader("Nodes")
        st.caption(
            "Nodes are the different events that exist across the process map. We count how many times each event occurs, which corresponds to the number of patients reaching each event."
        )
        st.dataframe(nodes, hide_index=True)
        st.subheader("Edges")
        st.caption(
            "Edges are the different paths that exist across the process map and the details associated with them."
        )
