...
with st.sidebar:
    ...
    parameters = dict(
        mean_patient_inter=st.slider(
            ...
            step=0.1,
            key="trial_iat",
            help="Average time between arrivals. Lower values mean more patients arrive.",
        ),
        num_receptionists=st.slider(
            "Number of receptionists",
            1,
            15,
            1,
            key="trial_receptionists",
            help="Number of patients who can be registered at the same time.",
        ),
        num_nurses=st.slider(
            "Number of nurses",
            1,
            15,
            1,
            key="trial_nurses",
            help="Number of patients who can see a nurse at the same time.",
        ),
        num_specialists=st.slider(
            ...
            15,
            1,
            key="trial_specialists",
            help="Number of patients who can see a specialist at the same time.",
        ),
        specialist_prob=st.slider(
            "Probability of needing a specialist",
            0.0,
            1.0,
            0.3,
            step=0.05,
            key="trial_specialist_prob",
            help="Chance of needing a specialist after seeing a nurse. 0.3 means 30%.",
        ),
    )
    simulation_hours = st.slider(
        ...
        8,
        key="trial_hours",
        help="Total length of each run, including any warm-up selected in a plot. Longer runs allow more patients to progress through the clinic.",
    )
    st.caption(
        "Simulation duration includes any plot warm-up. For example, an 8-hour run with a 1-hour warm-up leaves 7 hours of results to plot."
    )
    parameters.update(
        ...
        num_replications=st.slider(
            ...
            10,
            5,
            key="trial_replications",
            help="Independent runs with the same parameters and different seeds. More runs take longer to calculate.",
        ),
        mean_nurse_consult_time=10,
        sd_nurse_consult_time=4,
    )
    first_seed = st.number_input(
        "Random seed",
        1,
        100000,
        42,
        key="trial_seed",
        help="Each run uses a consecutive seed, beginning with this one. Keep it fixed when comparing parameter changes.",
    )

with st.expander("Interested in how the TrialLogger is built? Click here to find out."):
...
PAIRS = {
    "Waiting for receptionist": (
        "receptionist_wait_begins",
        "being_seen_by_receptionist",
    ),
    ...
    "Receptionist consultation": (
        "being_seen_by_receptionist",
        "receptionist_visit_ends",
    ),
    "Nurse consultation": ("being_seen_by_nurse", "nurse_treatment_ends"),
    "Specialist consultation": (
        "being_seen_by_specialist",
        "specialist_treatment_ends",
    ),
...
def render_queue_tab():
    ...
    with controls:
        ...
        selected = st.multiselect(
            "Queues",
            list(QUEUES),
            default=["Receptionist", "Nurse", "Specialist"],
            key="trial_queues",
            help="Choose which waiting queues to compare.",
        )
        all_runs = st.checkbox(
            "Show individual runs",
            True,
            key="trial_all_runs",
            help="Show individual simulation trajectories alongside the mean. Turn off to focus on the mean trajectory.",
        )
        shared = st.checkbox(
            "Share queue size axis",
            False,
            key="trial_shared_axis",
            help="Use the same vertical scale across queue panels for easier comparison.",
        )
        snapshot_column, warmup_column = st.columns(2)
        with snapshot_column:
            interval = st.slider(
                "Snapshot interval (mins)",
                1,
                15,
                5,
                key="trial_queue_interval",
                help="Time between queue measurements. Smaller values show more detail but take longer to calculate.",
            )
        with warmup_column:
            warm_up = st.slider(
                "Warm-up period (mins)",
                0,
                parameters["sim_duration"] - 30,
                0,
                step=30,
                key="trial_queue_warmup",
                help="Start plotting after this time. The simulation still runs from time zero so existing queues are retained.",
            )
        options = dict(
            event_list=[QUEUES[name] for name in selected],
...
def render_duration_tab():
    ...
    with controls:
        ...
        pair = st.selectbox(
            ...
            list(PAIRS),
            index=1,
            key="trial_duration_pair",
            help="Choose the start and end events for each patient duration.",
        )
        kind = st.selectbox(
            "Plot type",
            ["box", "violin", "ridgeline", "heatmap"],
            key="trial_duration_kind",
            help="Box: quartiles; violin: distribution shape; ridgeline and heatmap: compare distributions across runs.",
        )
        requires_split = kind in ("ridgeline", "heatmap")
        split = st.checkbox(
            "Separate runs",
            True,
            disabled=requires_split,
            key="trial_duration_split",
            help="Show each replication separately. Ridgeline and heatmap always separate runs.",
        )
        if kind in ("hist", "ridgeline", "heatmap"):
            bins = st.slider(
                ...
                20,
                key="trial_bins",
                help="Controls detail for histograms, ridgelines and heatmaps. More bins give narrower intervals.",
            )
        if kind in ("hist", "heatmap"):
            normalise = st.checkbox(
                ...
                False,
                key="trial_normalise",
                help="Scale histograms or heatmaps to density rather than counts. Ridgelines always use density.",
            )
        warm_up = st.slider(
            "Warm-up period (mins)",
            0,
            ...
            0,
            step=30,
            key="trial_duration_warmup",
            help="Exclude durations whose start event happened before this time.",
        )
        ...
        options = dict(
            ...
            second_event=end,
            kind=kind,
            split_by="run" if split or requires_split else None,
            warm_up=warm_up,
        )
        if kind in ("hist", "ridgeline", "heatmap"):
            options["bins"] = bins
    ...
    with output:
        st.caption(
            "Durations are in minutes. Only patients with both events recorded are included; unfinished waits or consultations are excluded. Compare the distributions across runs, then try another plot type."
        )
...
def render_resource_tab():
    ...
    with controls:
        ...
        metric = st.selectbox(
            "Measure",
            ["utilisation", "mean_in_use", "busy_time"],
            key="trial_resource_metric",
            help="Utilisation is the proportion of available capacity used; mean in use counts busy resource units; busy time totals their minutes in use.",
        )
        kind = st.selectbox(
            "Plot type",
            ["bar", "box", "violin"],
            key="trial_resource_kind",
            help="Bar shows a summary; box and violin show variation across replications.",
        )
        show_runs = st.checkbox(
            "Show individual runs",
            True,
            key="trial_resource_show_runs",
            help="Overlay results for each replication.",
        )
        if kind == "bar":
            error = st.selectbox(
                "Error bars",
                ["ci", "sd", "se", "range", "iqr", "None"],
                key="trial_resource_error",
                help="Choose how variation across runs is shown. CI is a 95% confidence interval; None removes error bars.",
            )
            sort = st.checkbox(
                "Sort by value",
                False,
                key="trial_resource_sort",
                help="Order steps from highest to lowest value.",
            )
        warm_up = st.slider(
            "Warm-up period (mins)",
            0,
            ...
            0,
            step=30,
            key="trial_resource_warmup",
            help="Exclude resource use before this time.",
        )
        options = dict(
            by="step",
            metric=metric,
...
def render_resource_time_tab():
    ...
    with controls:
        ...
        proportion = st.checkbox(
            "Show proportion of capacity",
            False,
            key="trial_resource_proportion",
            help="Divide the number of resource units in use by the number available at each step.",
        )
        show_runs = st.checkbox(
            "Show individual runs",
            True,
            key="trial_resource_time_runs",
            help="Show each replication as well as the mean trajectory.",
        )
        shared = st.checkbox(
            "Share vertical axis",
            True,
            key="trial_resource_shared_axis",
            help="Use the same y-axis scale for each resource step.",
        )
        snapshot_column, warmup_column = st.columns(2)
        with snapshot_column:
            interval = st.slider(
                ...
                15,
                5,
                key="trial_resource_interval",
                help="Time between measurements; smaller values show more detail.",
            )
        with warmup_column:
            warm_up = st.slider(
                "Warm-up period (mins)",
                ...
                0,
                step=30,
                key="trial_resource_time_warmup",
                help="Start plotting after this time while retaining resource use already underway.",
            )
        options = dict(
            every_x_time_units=interval,
            warm_up=warm_up,
            limit_duration=parameters["sim_duration"],
            as_proportion=proportion,
            show_all_runs=show_runs,
            shared_y_axis=shared,
        )
        if proportion:
...
def render_arrival_tab():
    ...
    with controls:
        ...
        pair = st.selectbox(
            ...
            list(PAIRS),
            index=1,
            key="trial_arrival_pair",
            help="Choose the two events whose elapsed time is plotted against patient arrival time.",
        )
        colour = st.selectbox(
            "Colour points by",
            ["run", "None"],
            key="trial_arrival_colour",
            help="Separate points by simulation run, or show all patients together.",
        )
        smoothing = st.selectbox(
            "Trend line",
            ["None", "Number of patients", "Minutes"],
            key="trial_arrival_smoothing",
            help="Add a moving average based on nearby patients or a time window.",
        )
        if smoothing == "Number of patients":
            window = st.slider(
                "Trend half-width (patients)",
                2,
                50,
                10,
                key="trial_arrival_window",
                help="Number of neighbouring patients on each side of a point used for the moving average.",
            )
        elif smoothing == "Minutes":
            time_window = st.slider(
                "Trend half-width (mins)",
                5,
                120,
                30,
                step=5,
                key="trial_arrival_time_window",
                help="Time on either side of an arrival used for the moving average.",
            )
        warm_up = st.slider(
            "Warm-up period (mins)",
            0,
            ...
            0,
            step=30,
            key="trial_arrival_warmup",
            help="Exclude patients arriving before this time.",
        )
        ...
        options = dict(
            ...
            colour_by=None if colour == "None" else colour,
            warm_up=warm_up,
        )
        if smoothing == "Number of patients":
